...
class Same extends format_js_1.Format {
    ...
    test() {
        const a = this.object;
        const b = this.expect;
        const so = this.options;
        return (so.notFound ? !!so.expectNotFound
    ...
    getPojoKeys(obj = this.object) {
        ...
        const pk = [
            ...new Set(fromSuper.concat(this.getPojoKeys(this.expect).filter(k => k in obj))),
        ];
        return pk;
    }
    printPojoHead() {
    ...
    printPojoEntry(key, val, notFound) {
        const expectNotFound = !hasOwnProperty.call(this.expect, key) &&
