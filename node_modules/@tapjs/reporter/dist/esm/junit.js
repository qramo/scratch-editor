...
class Suite {
    ...
    toString() {
        ...
        return `<testsuite name="${xmlEscape(this.name)}" tests="${this.tests}" failures="${this.failures}" assertions="${this.assertions}" skipped="${this.skipped}"${this.results?.time ? ` time="${seconds(this.results.time)}"` : ''}>
${props}
${this.suites
            .map(s => String(s))
            .join('\n')
